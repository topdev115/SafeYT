...
public class GetSubscriptionVideosTask extends AsyncTaskParallel<Void, Void, Void> {
	...
	private DateTime getFeedsLastUpdateTime() {
		long l = SafetoonsApp.getPreferenceManager().getLong(SafetoonsApp.KEY_SUBSCRIPTIONS_LAST_UPDATED, -1);
	...
	public static void updateFeedsLastUpdateTime(DateTime dateTime) {
		...
		editor.putLong(SafetoonsApp.KEY_SUBSCRIPTIONS_LAST_UPDATED, dateTime != null  ?  dateTime.getValue()  :  -1);
		editor.commit();
	}
