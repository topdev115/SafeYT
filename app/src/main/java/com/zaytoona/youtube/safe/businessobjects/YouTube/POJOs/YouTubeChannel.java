...
import com.zaytoona.youtube.safe.businessobjects.db.Tasks.SubscribeToChannelTask;
...
public class YouTubeChannel implements Serializable {
	...
	private void parse(Channel channel) {
		...
		if (branding != null)
			this.bannerUrl = SafetoonsApp.isTablet() ? branding.getImage().getBannerTabletHdImageUrl() : branding.getImage().getBannerMobileHdImageUrl();

		ChannelStatistics statistics = channel.getStatistics();
		if (statistics != null) {
			this.totalSubscribers = String.format(SafetoonsApp.getStr(R.string.total_subscribers),
	...
	private void getChannelInfoFromDB(boolean isUserSubscribed) {
		...
		this.isChannelAllowed = SubscriptionsDb.getSubscriptionsDb().isdChannelAllowed(this.getId());
	}
	...
	public void setChannelAllowed(boolean channelAllowed) {
		this.isChannelAllowed = channelAllowed;
	}
	...
	private boolean blacklistChannel(boolean displayToastMessage) {
		...
		if (displayToastMessage) {
			Toast.makeText(SafetoonsApp.getContext(),
