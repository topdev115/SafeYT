 ...
 */
public class MobileNetworkWarningDialog extends SafetoonsMaterialDialog {
	...
	public boolean showAndGetStatus(ActionType actionType) {
		final boolean   displayWarning  = SafetoonsApp.getPreferenceManager().getBoolean(SafetoonsApp.getStr(R.string.pref_key_warn_mobile_downloads), true);
		boolean         dialogDisplayed = false;

		if (SafetoonsApp.isConnectedToMobile() && displayWarning) {
			...
			checkBoxPromptRes(R.string.warning_mobile_network_disable, false, new CompoundButton.OnCheckedChangeListener() {
				...
				public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
					SafetoonsApp.getPreferenceManager().edit().putBoolean(SafetoonsApp.getStr(R.string.pref_key_warn_mobile_downloads), !isChecked).apply();
				}
			});
