...
public class ChannelBrowserFragment extends FragmentEx {
	...
	private Button 				allowChannelButton = null;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		allowChannelButton = fragment.findViewById(R.id.channel_allow_button);

		allowChannelButton.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				if (channel.isChannelAllowed()){
					SubscriptionsDb.getSubscriptionsDb().updateChannelAllowedStatus( channel.getId() , 0);
					channel.setChannelAllowed(false);
				}
				else {
					SubscriptionsDb.getSubscriptionsDb().updateChannelAllowedStatus( channel.getId() , 1);
					channel.setChannelAllowed(true);
				}
	...
	private void initViews() {
		if (channel != null) {
			...
			if (channel.isChannelAllowed()) {
				allowChannelButton.setText(R.string.dont_allow_button);
			} else {
				allowChannelButton.setText(R.string.allow_button);
			}
	 ...
	 * @return {@link com.zaytoona.youtube.safe.gui.fragments.ChannelPlaylistsFragment}
	 */
